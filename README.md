...
Note: The current setup runs the backend as a separate process (e.g., you can run it with Bun as shown above). If you want the Tauri app to spawn/manage the Bun server automatically on app launch and shut it down on exit, that can be added via Tauri v2’s process APIs in [web/src-tauri/src/main.rs](web/src-tauri/src/main.rs:11).
